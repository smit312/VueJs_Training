...
export default defineComponent({
  components: { AlertBox, ValidationProvider, ValidationObserver },
  middleware: ["notAuth"],
  setup() {
    ...
    let errmsg = reactive("");
    let successmsg = reactive("");

    //methods
    async function onSubmit(event) {
      event.preventDefault();
      const res = await store.dispatch("auth/loginUser", form);
      if (res === "success") {
        router.push("/Home");
      }
      form = {
    ...
    return { form, onSubmit, isLoading, errmsg, successmsg };
  },
});
</script>
